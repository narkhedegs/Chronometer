...
namespace Narkhedegs.PerformanceMeasurement
{
    ...
    internal class NormalizedMeanCalculator : INormalizedMeanCalculator
    {
        ...
        public double Calculate(IEnumerable<double> values)
        {
            ...
            if (enumerable.Any())
            {
                var deviations = enumerable.Select(value => Tuple.Create(value, enumerable.Average() - value)).ToList();
                var meanDeviation = deviations.Sum(deviation => Math.Abs(deviation.Item2))/enumerable.Count();
                response = deviations.Where(tuple => tuple.Item2 > 0 || Math.Abs(tuple.Item2) <= meanDeviation)
                    .Average(deviation => deviation.Item1);
            }
